...
def host_search_cmd(
    ...
    if export is not None:
        if not os.path.isdir(os.path.dirname(export)):
            click.echo(click.style(f"{os.path.dirname(export)} is not a valid directory", fg='red'))
            return
        ...
        with open(export, 'w', newline='', encoding='utf-8') as csv_file_handle:
            ...
            click.echo(click.style(
                f"Successfully exported host data for {len(host_data)} hosts to {export}",
                fg='green')
            )
